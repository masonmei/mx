...
public abstract class Unmarshaller<T>
{
  ...
  public abstract T unmarshall(Node paramNode)
    ...
    unmarshallers.put(String.class, new Unmarshaller(String.class)
    {
      public String unmarshall(Node node)
      {
        if (node.getNodeType() == 1) {
          ...
          if (firstChild == null) {
            throw new DOMException((short)8, MessageFormat.format("The element node {0}  is present, but is empty.", new Object[] { node.getNodeName() }));
          }
