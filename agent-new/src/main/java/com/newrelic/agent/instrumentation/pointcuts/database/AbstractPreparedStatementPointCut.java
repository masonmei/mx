...
public abstract class AbstractPreparedStatementPointCut extends TracerFactoryPointCut
{
  ...
  static final MethodMatcher METHOD_MATCHER = OrMethodMatcher.getMethodMatcher(new MethodMatcher[] { new ExactMethodMatcher("executeQuery", "()Ljava/sql/ResultSet;"), new ExactMethodMatcher("executeUpdate", "()I"), new ExactMethodMatcher("execute", "()Z") });
  ...
  public final Tracer doGetTracer(Transaction transaction, ClassMethodSignature sig, Object preparedStatement, Object[] args)
  {
    if ((preparedStatement instanceof PreparedStatementExtension)) {
      ...
      if (statementData != null) {
        if (this.logger.isLoggable(Level.FINEST)) {
          String msg = MessageFormat.format("Created PreparedStatementTracer for: {0}", new Object[] { preparedStatement.getClass().getName() });
      ...
      if (this.logger.isLoggable(Level.FINEST)) {
        try {
          String msg = MessageFormat.format("Statement data is null: {0},{1}", new Object[] { sig, ((PreparedStatement)preparedStatement).getConnection().getClass().getName() });
      ...
      String msg = MessageFormat.format("PreparedStatement does not implement PreparedStatementExtension: {0}", new Object[] { preparedStatement.getClass().getName() });
    ...
    throws Exception
  {
    ...
    int i = 0; for (int j = 1; i < pieces.length; j++) {
