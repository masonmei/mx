...
public class HttpURLConnectionPointCut extends TracerFactoryPointCut {
    ...
    private static ClassMatcher createClassMatcher() {
        return new ChildClassMatcher("java/net/HttpURLConnection", false,
                                            new String[] {"sun/net/www/protocol/http/HttpURLConnection"});
    }

    private static MethodMatcher createMethodMatcher() {
        return OrMethodMatcher.getMethodMatcher(new MethodMatcher[] {new ExactMethodMatcher("connect", "()V"),
                                                                            new ExactMethodMatcher("getResponseCode",
                                                                                                          "()I"),
                                                                            new ExactMethodMatcher("getInputStream",
                                                                                                          "()Ljava/io/InputStream;"),
                                                                            new ExactMethodMatcher("getOutputStream",
    ...
    public void noticeTransformerStarted(ClassTransformer classTransformer) {
        try {
            ServiceFactory.getAgent().getInstrumentation()
                    .retransformUninstrumentedClasses(new String[] {"java/net/HttpURLConnection".replaceAll("/", ".")});
        } catch (Throwable t) {
            String msg = MessageFormat.format("Error retransforming {0}: {1}",
                                                     new Object[] {"java/net/HttpURLConnection", t});
            Agent.LOG.info(msg);
        }
    }
    ...
    public Tracer doGetTracer(Transaction transaction, ClassMethodSignature sig, Object connection, Object[] args) {
        ...
        Agent.LOG.log(Level.FINEST, "HttpUrlConnection getTransaction {0}: connected : {1}, new relic header: {2}",
                             new Object[] {sig.getMethodName(), Boolean.valueOf(connected), idHeader});
    ...
    private Tracer getConnectionTracer(Transaction transaction, ClassMethodSignature sig,
                                       HttpURLConnection urlConnection, String uri, String host, String methodName) {
        ...
        return new ExternalComponentTracer(transaction, sig, urlConnection, host, "HttpURLConnection", uri,
                                                  new String[] {methodName});
    }

    private Tracer getOutputStreamConnectionTracer(Transaction transaction, ClassMethodSignature sig,
                                                   HttpURLConnection urlConnection, String uri, String host,
                                                   String methodName) {
        final URL url = urlConnection.getURL();
        transaction.getCrossProcessState().processOutboundRequestHeaders(new OutboundHeadersWrapper(urlConnection));
        return new ExternalComponentTracer(transaction, sig, urlConnection, host, "HttpURLConnection", uri,
    ...
    private Tracer getResponseCodeTracer(Transaction transaction, ClassMethodSignature sig,
                                         HttpURLConnection urlConnection, String uri, String host, String methodName) {
