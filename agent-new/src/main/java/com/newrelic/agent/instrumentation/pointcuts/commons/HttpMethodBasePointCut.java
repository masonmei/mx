...
public class HttpMethodBasePointCut extends HttpCommonsPointCut
{
  ...
  protected Tracer getExternalTracer(Transaction transaction, ClassMethodSignature sig, Object httpMethod, Object[] args)
  {
    String host = "Unknown";
    String methodName = "";
    String uri = "";
    ...
    try {
      if (sig.getMethodName() == "execute") {
        ...
        if ((httpMethod instanceof HttpMethodBase)) {
          ...
          transaction.getCrossProcessState().processOutboundRequestHeaders(new OutboundHeadersWrapper(httpMethodBase));
        }
    ...
    {
      String msg = MessageFormat.format("Instrumentation error invoking {0} in {1}: {2}", new Object[] { sig, getClass().getName(), t });
  ...
  private String getHost(Object httpConnection)
    throws Exception
  {
  ...
  private String getScheme(Object httpConnection)
    throws Exception
  {
  ...
  private int getPort(Object httpConnection)
    throws Exception
  {
    if ((httpConnection instanceof HttpConnection)) {
  ...
  private URI getUri(Object httpMethod)
    throws Exception
  {
