  ...
  implements ContextClassTransformer
{
  ...
  Collection<PointCut> findEnabledPointCuts() {
    Collection<Class<?>> classes = Annotations.getAnnotationClassesFromManifest(com.newrelic.agent.instrumentation
                                                                                      .pointcuts
                                                                               .PointCut.class, "com/newrelic/agent/instrumentation/pointcuts");
  ...
  private PointCut createPointCut(Class<PointCut> clazz) {
    try {
      return (PointCut)clazz.getConstructor(new Class[] { ClassTransformer.class }).newInstance(new Object[] { this });
    } catch (Exception e) {
      String msg = MessageFormat.format("Unable to create pointcut {0} : {1}", new Object[] { clazz.getName(), e.toString() });
    ...
    throws IllegalClassFormatException
  {
    ...
    try
    {
      ...
      this.logger.severe(MessageFormat.format("An error occurred processing class {0} : {1}", new Object[] { className, e.toString() }));
  ...
  protected boolean shouldTransform(ClassLoader loader, String className, byte[] classfileBuffer)
  {
    ...
    if (isExcluded(className)) {
      if (isLoggable) {
        this.logger.finest(MessageFormat.format("Skipping class {0} because it is excluded", new Object[] { className }));
      }
      return false;
    }
    ...
    if (className.startsWith("$")) {
      if (isLoggable) {
        this.logger.finest(MessageFormat.format("Skipping class {0} because it starts with $", new Object[] { className }));
      }
      return false;
    }
    ...
    if ((className.indexOf("$$") > 0) && (!className.startsWith("play"))) {
      if (isLoggable) {
        this.logger.finest(MessageFormat.format("Skipping class {0} because it contains $$ and is not a Play class", new Object[] { className }));
      }
    ...
    if (isValidClassByteArray(classfileBuffer)) {
      if (isLoggable) {
        this.logger.finest(MessageFormat.format("Skipping class {0} because it does not appear to be a valid class file", new Object[] { className }));
      }
    ...
    if ((loader == null) && (!isBootstrapClassInstrumentationEnabled())) {
      if (isLoggable) {
        this.logger.finest(MessageFormat.format("Skipping class {0} because bootstrap class instrumentation is not supported", new Object[] { className }));
      }
  ...
  class WeavingLoaderImpl
  {
    ...
    public byte[] preProcess(InstrumentationContext context, String className, Class<?> classBeingRedefined, byte[] classfileBuffer, Match match)
    {
      ...
      if ((this.classLoader != null) && (!InstrumentationUtils.isAbleToResolveAgent(this.classLoader, className))) {
        String msg = MessageFormat.format("Not instrumenting {0}: class loader unable to load agent classes", new Object[] { className });
      ...
      catch (StopProcessingException e)
      {
        ...
        String msg = MessageFormat.format("Skipping transformation of class {0} ({1} bytes) because an ASM array bounds exception occurred: {2}", new Object[] { className, Integer.valueOf(classfileBuffer.length), t.toString() });
        ...
        ClassTransformer.this.logger.warning(MessageFormat.format("Skipping transformation of class {0} because an error occurred: {1}", new Object[] { className, t.toString() }));
    ...
    private synchronized void redefineClass(String className, byte[] classfileBuffer)
    {
      try {
        ...
        String msg = MessageFormat.format("An error occurred redefining class {0}: {1}", new Object[] { className, e });
