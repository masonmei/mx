...
public class SolrRequestHandlerPointCut extends AbstractSolrPointCut
{
        ...
        if (policy.canSetTransactionName(tx, TransactionNamePriority.FRAMEWORK)) {
          ...
          if (uri == null) {
            ...
            if (Agent.LOG.isLoggable(Level.FINER)) {
              String msg = MessageFormat.format("Setting transaction name to \"{0}\" using Solr request URI", new Object[] { uri });
  ...
  static Map<String, String> addDebugInfo(Object rb)
  {
    ...
    try
    {
      ...
      String rawQueryString = (String)solrParams.getClass().getMethod("get", new Class[] { String.class }).invoke(solrParams, new Object[] { "q" });
      ...
      String parsedQuery = (String)helper.queryParsingClass.getMethod("toString", new Class[] { helper.queryClass, helper.indexSchemaClass }).invoke(null, new Object[] { query, schema });
  ...
  private String getQueryType(Object handler, Object request) {
    try {
      ...
      String msg = MessageFormat.format("Unable to get the SolrQueryRequest query type : {0}", new Object[] { e.toString() });
      Agent.LOG.info(msg);
      Agent.LOG.log(Level.FINER, msg, e);
    }
    return null; } 
  private static class SolrReflectionHelper { public final Class<?> solrPluginUtilsClass;
    ...
    public SolrReflectionHelper(Object rb) throws Exception { Class responseBuilderClass = rb.getClass();
