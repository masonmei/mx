...
public class PointCutFactory
{
  ...
  public ExtensionClassAndMethodMatcher getPointCut(Object obj) throws ParseException {
    ...
    throw new RuntimeException(MessageFormat.format("Unknown pointcut type: {0} ({1}", new Object[] { obj, obj.getClass().getName() }));
  }

  public ExtensionClassAndMethodMatcher getPointCut(String string)
  ...
  public List<ExtensionClassAndMethodMatcher> getPointCuts(Map namesToPointCuts) throws ParseException
  {
    ...
    List pcs = new ArrayList();
    for (Iterator i$ = values.iterator(); i$.hasNext(); ) { Object obj = i$.next();
      if ((obj instanceof String))
  ...
  static Collection<ClassMatcher> getClassMatchers(Collection matchers) {
    ...
    for (Iterator i$ = matchers.iterator(); i$.hasNext(); ) { Object matcher = i$.next();
  ...
  static MethodMatcher getMethodMatcher(Object yaml) {
    ...
    if ((yaml instanceof MethodMatcher)) {
      matcher = (MethodMatcher)yaml; } else {
      if ((yaml instanceof List)) {
        List list = (List)yaml;
  ...
  private static MethodMatcher validateMethodMatcher(ExactMethodMatcher methodMatcher) {
    try {
      ...
      Agent.LOG.log(Level.SEVERE, MessageFormat.format("The method matcher can not be created, meaning the methods associated with it will not be monitored - {0}", new Object[] { e.toString() }));
