...
public abstract class PointCut
  implements Comparable<PointCut>, ClassAndMethodMatcher
{
  ...
  protected PointCut(PointCutConfiguration config, ClassMatcher classMatcher, MethodMatcher methodMatcher)
  {
    ...
    this.config = config;
    this.isIgnoreTransaction = ((Boolean)config.getConfiguration().getProperty("ignore_transaction", Boolean.valueOf(false))).booleanValue();
  }
  ...
  protected void logInstrumentation(String className, Class<?> classBeingRedefined) {
    if (Agent.isDebugEnabled())
      Agent.LOG.finer(MessageFormat.format("Instrumenting {0} {1}", new Object[] { className, classBeingRedefined == null ? "" : "(Second pass)" }));
  }
  ...
  {
    return ComparisonChain.start().compare(pc.getPriority(), getPriority()).compare(getClass().getName(), pc.getClass().getName()).result();
  }
