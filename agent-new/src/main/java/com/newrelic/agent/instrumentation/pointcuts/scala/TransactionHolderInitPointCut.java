...
public class TransactionHolderInitPointCut extends com.newrelic.agent.instrumentation.PointCut implements
    ...
    public TransactionHolderInitPointCut(ClassTransformer classTransformer) {
        ...
        this.tracerNamingEnabled =
                ServiceFactory.getConfigService().getTransactionTracerConfig(null).isStackBasedNamingEnabled();
    }
    ...
    public void handleInvocation(ClassMethodSignature sig, Object object, Object[] args) {
        if ((object instanceof TransactionHolder)) {
            ...
            if ((tx.isStarted()) && (th._nr_getTransaction() == null)) {
                if ((!(tx.getTransactionState() instanceof AsyncTransactionState)) || (tx.getRootTransaction()
                                                                                               .isIgnore())) {
                    return;
                }
                                        ...
                                        if (!st.getClassName().startsWith("play.api.libs.iteratee")) {
                                            if (!st.getClassName().startsWith("play.libs.F")) {
                                                if (!st.getClassName().startsWith("akka.pattern.PromiseActorRef")) {
                                                    if (!st.getClassName().startsWith("java.util.concurrent"
                                                                                              + ".ThreadPoolExecutor")) {
                                                        if (!st.getClassName().startsWith("java.lang.Thread")) {
                                                            return MessageFormat.format("Java/{0}/{1}",
                                                                                               new Object[] {st.getClassName(),
                                                                                                                    st.getMethodName()});
                                                        }
                                                    }
                                                }
        ...
        for (Field field : onComplete.getClass().getDeclaredFields()) {
            if (field.getType().getName().startsWith("scala.Function")) {
                ...
                try {
                    ...
                    if ((callback.getClass().getName().startsWith("scala.concurrent")) || (callback.getClass().getName()
                                                                                                   .startsWith("play.api.libs.concurrent"))
                                || (callback.getClass().getName().startsWith("play.api.libs.iteratee"))) {
