...
public class InterfaceMatcher extends ClassMatcher
{
    ...
    if ((superName != null) && (!superName.equals("java/lang/Object"))) {
      try {
        ...
        if (Agent.LOG.isFinestEnabled())
          Agent.LOG.finest(MessageFormat.format("Unable to load class {0}: {1}", new Object[] { superName, e }));
      }
  ...
  private boolean isInterfaceMatch(ClassLoader loader, String interfaceName)
  {
    try {
      ...
      if (Agent.LOG.isFinestEnabled()) {
        Agent.LOG.finest(MessageFormat.format("Unable to load interface {0}: {1}", new Object[] { interfaceName, e }));
      }
      return false;
