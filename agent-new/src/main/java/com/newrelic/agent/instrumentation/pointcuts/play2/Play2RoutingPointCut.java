...
public class Play2RoutingPointCut extends com.newrelic.agent.instrumentation.PointCut implements
    ...
    private void setTransactionName(Transaction tx, String controllerAction) {
        ...
        if ((Agent.LOG.isLoggable(Level.FINER)) && (policy.canSetTransactionName(tx,
                                                                                        TransactionNamePriority
                                                                                                .FRAMEWORK_LOW))) {
            String msg = MessageFormat.format("Setting transaction name to \"{0}\" using Play 2 controller action",
