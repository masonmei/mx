...
public class ParameterizedPreparedStatementPointCut extends com.newrelic.agent.instrumentation.PointCut
  implements EntryInvocationHandler
{
  ...
  private static final MethodMatcher METHOD_MATCHER = OrMethodMatcher.getMethodMatcher(new MethodMatcher[] { new ExactMethodMatcher("setInt", "(II)V"), new ExactMethodMatcher("setBoolean", "(IZ)V"), new ExactMethodMatcher("setByte", "(IB)V"), new ExactMethodMatcher("setShort", "(IS)V"), new ExactMethodMatcher("setLong", "(IJ)V"), new ExactMethodMatcher("setFloat", "(IF)V"), new ExactMethodMatcher("setDouble", "(ID)V"), new ExactMethodMatcher("setBigDecimal", "(ILjava/math/BigDecimal;)V"), new ExactMethodMatcher("setString", "(ILjava/lang/String;)V"), new ExactMethodMatcher("setDate", "(ILjava/sql/Date;)V"), new ExactMethodMatcher("setTime", "(ILjava/sql/Time;)V"), new ExactMethodMatcher("setTimestamp", "(ILjava/sql/Timestamp;)V"), new ExactMethodMatcher("setNull", "(II)V") });
  private final IAgentLogger logger;

  public ParameterizedPreparedStatementPointCut(ClassTransformer classTransformer)
  {
    super(new PointCutConfiguration("jdbc_parameterized_prepared_statement", null, ServiceFactory.getConfigService().getDefaultAgentConfig().isGenericJDBCSupportEnabled()), createClassMatcher(), METHOD_MATCHER);
      ...
      if (params != null)
      {
        try
        {
          ...
          if (this.logger.isLoggable(Level.FINE)) {
            String msg = MessageFormat.format("Instrumentation error for {0} in {1}: {2}", new Object[] { sig.toString(), ParameterizedPreparedStatementPointCut.class.getName(), e.toString() });
