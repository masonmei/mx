...
public class DeadLockDetector
{
  ...
  protected void detectDeadlockedThreads() {
    ...
    if (threadInfos.length > 0) {
      Agent.LOG.info(MessageFormat.format("Detected {0} deadlocked threads", new Object[] { Integer.valueOf(threadInfos.length) }));
  ...
  protected long[] findDeadlockedThreads() {
    try {
      return getThreadMXBean().findDeadlockedThreads(); } catch (UnsupportedOperationException e) {
    }
