...
public class RPMService extends AbstractService
  implements IRPMService, EnvironmentChangeListener, AgentConfigListener
{
  ...
  public RPMService(List<String> appNames, ConnectionListener connectionListener) {
    ...
    this.appName = ((String)appNames.get(0)).intern();
  ...
  public synchronized Map<String, Object> launch()
    throws Exception
  {
    if (isConnected()) {
      return null;
    ...
    Map data = null;
    try {
      data = this.dataSender.connect(getStartOptions());
    ...
    if (!data.keySet().containsAll(requiredParams)) {
      throw new UnexpectedException(MessageFormat.format("Missing the following connection parameters", new Object[] { Boolean.valueOf(requiredParams.removeAll(data.keySet())) }));
    }
  ...
  private void logCollectorMessages(Map<String, Object> data) {
    List<Map> messages = (List)data.get("messages");
    if (messages != null)
  ...
  {
    return MessageFormat.format("{0}:{1}", new Object[] { this.host, Integer.toString(this.port) });
  }
  ...
  public List<Long> sendProfileData(List<IProfile> profiles)
    throws Exception
  {
  ...
  public void sendModules(List<Jar> pJarsToSend)
    throws Exception
  {
  ...
  private void queuePingCommandSyncRestart()
    throws Exception
  {
    try
    {
  ...
  {
    List errors = this.errorService.harvest(this, statsEngine);
    ...
    if (isConnected()) {
      ...
      List data = statsEngine.getMetricData(metricNormalizer, this.metricIdRegistry);
      long startTime = System.nanoTime();
      long reportInterval = 0L;
      try {
        long now = System.currentTimeMillis();
        List responseList = sendMetricDataSyncRestart(this.lastReportTime, now, data);
        ...
        Agent.LOG.log(Level.FINE, "Reported {0} timeslices for {1}", new Object[] { Integer.valueOf(data.size()), getApplicationName() });

        sendErrorData(errors);
      } catch (InternalLimitExceeded e) {
        Agent.LOG.log(Level.SEVERE, "The metric data post was too large.  {0} timeslices will not be resent", new Object[] { Integer.valueOf(data.size()) });
      }
        ...
        {
          Agent.LOG.log(Level.INFO, "An error occurred posting metric data - {0}.  This data will be resent later.", new Object[] { e.getMessage() });
        }
        else
        {
          Agent.LOG.log(Level.SEVERE, "An error occurred posting metric data - {0}.  {1} timeslices will not be resent.", new Object[] { e.getMessage(), Integer.valueOf(data.size()) });
        }
      }
      catch (ForceRestartException e)
      {
        ...
        retry = true;
        Agent.LOG.log(Level.INFO, "An connection error occurred contacting {0}.  Please check your network / proxy settings.", new Object[] { e.getHost() });
  ...
  private void registerMetricIds(List<List<?>> responseList)
  {
    for (List response : responseList) {
      ...
      this.metricIdRegistry.setMetricId(metricName, Integer.valueOf(id.intValue()));
    }
  }
