...
abstract class CustomInsightsEventsConfigUtils {
    ...
    static boolean isCustomInsightsEventsEnabled(AgentConfig config, int maxSamplesStored) {
        ...
        Boolean featureGateEnabled =
                (Boolean) config.getValue("custom_insights_events.collect_custom_events", Boolean.valueOf(true));
        return (notHighSecurity) && (storedMoreThan0) && (configEnabled.booleanValue()) && (featureGateEnabled
                                                                                                    .booleanValue());
    }

    static int getMaxSamplesStored(AgentConfig config) {
        return ((Integer) config.getValue("custom_insights_events.max_samples_stored", Integer.valueOf(10000)))
                       .intValue();
    }
}