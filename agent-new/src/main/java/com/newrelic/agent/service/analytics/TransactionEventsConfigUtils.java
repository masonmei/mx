...
abstract class TransactionEventsConfigUtils
{
  ...
  {
    return (maxSamplesStored > 0) && (((Boolean)config.getValue("analytics_events.enabled", Boolean.valueOf(true))).booleanValue()) && (((Boolean)config.getValue("transaction_events.enabled", Boolean.valueOf(true))).booleanValue()) && (((Boolean)config.getValue("transaction_events.collect_analytics_events", Boolean.valueOf(true))).booleanValue());
  }
