...
public class TransactionEventsService extends AbstractService implements Service, HarvestListener, TransactionListener,
    ...
    public TransactionEventsService() {
        ...
        this.isEnabledForApp.put(config.getApplicationName(), Boolean.valueOf(this.enabled));
        this.transactionNameCache =
                CacheBuilder.newBuilder().maximumSize(this.maxSamplesStored).expireAfterAccess(5L, TimeUnit.MINUTES)
    ...
    public void beforeHarvest(String appName, StatsEngine statsEngine) {
        ...
        List reservoirToSend =
                (List) this.reservoirForApp.put(appName, new ReserviorSampledArrayList(this.maxSamplesStored));
        ...
        if ((reservoirToSend != null) && (reservoirToSend.size() > 0)) {
            try {
                ...
                ReserviorSampledArrayList currentReservoir =
                        (ReserviorSampledArrayList) this.reservoirForApp.get(appName);
    ...
    private void beforeHarvestSynthetics(String appName, StatsEngine statsEngine) {
        FixedSizeArrayList current =
                (FixedSizeArrayList) this.syntheticsListForApp.put(appName, new FixedSizeArrayList(200));
        ...
        for (int nSent = 0; nSent < 5; nSent++) {
            FixedSizeArrayList toSend = (FixedSizeArrayList) this.pendingSyntheticsArrays.poll();
            if (toSend == null) {
                break;
            }
            try {
    ...
    public void dispatcherTransactionFinished(TransactionData transactionData, TransactionStats transactionStats) {
        ...
        if (transactionData.isSyntheticTransaction()) {
            FixedSizeArrayList currentSyntheticsList = (FixedSizeArrayList) this.syntheticsListForApp.get(name);
            while (currentSyntheticsList == null) {
                this.syntheticsListForApp.putIfAbsent(name, new FixedSizeArrayList(200));

                currentSyntheticsList = (FixedSizeArrayList) this.syntheticsListForApp.get(name);
            }
        ...
        if (!persisted) {
            ReserviorSampledArrayList currentReservoir = (ReserviorSampledArrayList) this.reservoirForApp.get(name);
            while (currentReservoir == null) {
                this.reservoirForApp.putIfAbsent(name, new ReserviorSampledArrayList(this.maxSamplesStored));
                currentReservoir = (ReserviorSampledArrayList) this.reservoirForApp.get(name);
            }
    ...
    private TransactionEvent createEvent(TransactionData transactionData, TransactionStats transactionStats) {
        ...
        try {
            metricName = (String) this.transactionNameCache.get(metricName);
    ...
    public ReserviorSampledArrayList<TransactionEvent> unsafeGetEventData(String appName) {
        return (ReserviorSampledArrayList) this.reservoirForApp.get(appName);
    }
