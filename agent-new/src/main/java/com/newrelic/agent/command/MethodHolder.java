...
class MethodHolder
{
  ...
  protected boolean isMethodPresent(String name, String descr, boolean remove)
  {
    ...
    if (value != null) {
      if (this.isDebug) {
        XmlInstrumentValidator.printMessage(MessageFormat.format("Found the method {0} from the xml in the list of class methods. Checking method parameters.", new Object[] { name }));
      }
      ...
      while (it.hasNext()) {
        ...
        if (descr.startsWith(xmlDesc)) {
          XmlInstrumentValidator.printMessage(MessageFormat.format("Matched Method: {0} {1}", new Object[] { name, descr }));
          ...
          XmlInstrumentValidator.printMessage(MessageFormat.format("Descriptors for method {0} did not match. Xml descriptor: {1}, Method descriptor: {2} ", new Object[] { name, xmlDesc, descr }));
        }
      }
    }
