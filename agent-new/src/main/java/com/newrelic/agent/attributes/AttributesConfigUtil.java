...
{
  protected static final String[] DEFAULT_BROWSER_EXCLUDES = { "request.parameters.*", "message.parameters.*", "library.solr.*", "jvm.*", "httpResponseMessage", "request.headers.referer", "httpResponseCode", "host.displayName", "process.instanceName" };

  protected static final String[] DEFAULT_EVENTS_EXCLUDES = { "request.parameters.*", "message.parameters.*", "library.solr.*", "jvm.*", "httpResponseMessage", "request.headers.referer" };
  ...
  private static Boolean getBooleanValue(AgentConfig config, String value, Object theDefault) {
    try {
      ...
      Agent.LOG.log(Level.FINE, MessageFormat.format("The configuration property {0} should be a boolean but is not.", new Object[] { value }));
    }
