  ...
  implements MetricNameFormat
{
  ...
  public String getHostCrossProcessIdRollupMetricName() {
    return Strings.join('/', new String[] { "ExternalApp", this.hostName, this.crossProcessId, "all" });
  }
  ...
  {
    return Strings.join('/', new String[] { "ExternalTransaction", this.hostName, this.crossProcessId, this.transactionName });
  }
  ...
  public static CrossProcessNameFormat create(String host, String uri, String decodedAppData) {
    ...
    try
    {
      ...
      if (Agent.LOG.isFinerEnabled()) {
        String msg = MessageFormat.format("Unable to parse application data {0}: {1}", new Object[] { decodedAppData, ex });
        Agent.LOG.finer(msg);
      }
    }
    return null;
  ...
  public static CrossProcessNameFormat create(String host, String uri, String encodedAppData, String encodingKey)
  {
    ...
    try {
      ...
      String msg = MessageFormat.format("Error decoding application data {0}: {1}", new Object[] { encodedAppData, ex });
      Agent.LOG.error(msg);
      return null;
    }
