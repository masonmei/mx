  ...
  public final Object invoke(Object methodName, Method method, Object[] args)
  {
    try {
      if (args == null)
        ...
        if (args.length == 2)
          ...
          Agent.LOG.severe(MessageFormat.format("Tracer.finish(int, Object) was invoked with {0} arguments(s)", new Object[] { Integer.valueOf(args.length) }));
        }
      }
        ...
        if (args.length == 1)
          ...
          Agent.LOG.severe(MessageFormat.format("Tracer.finish(Throwable) was invoked with {0} arguments(s)", new Object[] { Integer.valueOf(args.length) }));
        }
      }
      else
        Agent.LOG.severe(MessageFormat.format("Tracer.finish was invoked with an unknown method: {0}", new Object[] { methodName }));
    }
    catch (RetryException e)
    {
      ...
      if (Agent.LOG.isLoggable(Level.FINE)) {
        String msg = MessageFormat.format("An error occurred finishing method tracer {0} for signature {1} : {2}", new Object[] { getClass().getName(), getClassMethodSignature(), t.toString() });
