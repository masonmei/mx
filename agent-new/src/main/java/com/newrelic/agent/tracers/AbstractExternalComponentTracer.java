...
public abstract class AbstractExternalComponentTracer extends DefaultTracer implements IgnoreChildSocketCalls {
    ...
    public AbstractExternalComponentTracer(Transaction transaction, ClassMethodSignature sig, Object object,
                                           String host, String library, boolean includeOperationInMetric, String uri,
                                           String[] operations) {
        super(transaction, sig, object,
                     ExternalComponentNameFormat.create(host, library, includeOperationInMetric, uri, operations));
    ...
    protected void doRecordMetrics(TransactionStats transactionStats) {
        ...
        transactionStats.getUnscopedStats().getResponseTimeStats("External/all")
                .recordResponseTime(getExclusiveDuration(), TimeUnit.NANOSECONDS);

        transactionStats.getUnscopedStats()
                .getResponseTimeStats(getTransaction().isWebTransaction() ? "External/allWeb" : "External/allOther")
                .recordResponseTime(getExclusiveDuration(), TimeUnit.NANOSECONDS);

        String hostRollupMetricName = Strings.join('/', new String[] {"External", getHost(), "all"});
        transactionStats.getUnscopedStats().getResponseTimeStats(hostRollupMetricName)
                .recordResponseTime(getExclusiveDuration(), TimeUnit.NANOSECONDS);
    }
}