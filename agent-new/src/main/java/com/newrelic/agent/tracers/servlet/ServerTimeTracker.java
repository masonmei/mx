...
public class ServerTimeTracker
{
  ...
  private ServerTimeTracker(Request httpRequest, long txStartTimeInNanos, long queueTime)
  {
    ...
    if (requestXStartHeader == null) {
      ...
      this.totalServerTime = initRequestTime(requestXStartHeader, txStartTimeInNanos, queueTime);
    }
  }
  ...
  private long initRequestTime(String requestXStartHeader, long txStartTimeInNanos, long queueTime) {
      ...
      try {
        ...
        if (lastServerName != null) {
          ...
          if (Agent.LOG.isLoggable(Level.FINEST)) {
            String msg = MessageFormat.format("{0} start time (nanoseconds): {1}, {2} start time (in nanoseconds): {3}, {4} time (in nanoseconds): {5}", new Object[] { serverName, Long.valueOf(serverStartTimeInNanos), lastServerName, Long.valueOf(lastServerStartTimeInNanos), lastServerName, Long.valueOf(serverTimeInNanos) });

            Agent.LOG.finest(msg);
          }
          if (serverTimeInNanos > 0L) {
            this.serverTimes.put(lastServerName, Long.valueOf(serverTimeInNanos));
        ...
        String msg = MessageFormat.format("Error parsing server time {0} in {1}: {2}", new Object[] { serverStartTime, "X-Request-Start", e });
    ...
    if (lastServerName != null) {
      ...
      if (Agent.LOG.isLoggable(Level.FINEST)) {
        String msg = MessageFormat.format("Transaction start time (nanoseconds): {0}, {1} start time (in nanoseconds): {2}, queue time (in nanoseconds): {3}, {4} time (in nanoseconds): {5}", new Object[] { Long.valueOf(txStartTimeInNanos), lastServerName, Long.valueOf(lastServerStartTimeInNanos), Long.valueOf(queueTime), lastServerName, Long.valueOf(serverTimeInNanos) });
  ...
  public void recordMetrics(TransactionStats statsEngine) {
    if (this.totalServerTime > 0L) {
      recordAllServerTimeMetric(statsEngine, this.totalServerTime);
      for (Entry entry : this.serverTimes.entrySet())
  ...
  private void recordServerTimeMetric(TransactionStats statsEngine, String serverName, long serverTime)
  {
    ...
    if (Agent.LOG.isLoggable(Level.FINEST)) {
      String msg = MessageFormat.format("Recorded metric: {0}, value: {1}", new Object[] { name, String.valueOf(serverTime) });
      Agent.LOG.finest(msg);
    }
  }
