  ...
  implements InboundHeaders
{
  ...
  protected void doRecordCrossProcessRollup(TransactionStats transactionStats) {
    if (this.crossProcessFormat != null)
    {
      ...
      transactionStats.getUnscopedStats().getResponseTimeStats(hostCrossProcessIdRollupMetricName).recordResponseTime(getExclusiveDuration(), TimeUnit.NANOSECONDS);
    }
  }
