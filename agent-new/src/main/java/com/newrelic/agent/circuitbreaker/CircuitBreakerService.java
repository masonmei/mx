...
public class CircuitBreakerService extends AbstractService
  implements HarvestListener, AgentConfigListener
{
  ...
  public CircuitBreakerService()
  {
    ...
    {
      Agent.LOG.log(Level.WARNING, "Circuit breaker: Missing required JMX beans. Cannot enable circuit breaker. GC bean: {0} CpuTime: {1}", new Object[] { getOldGenGCBean(), Long.valueOf(getCpuTimeNS()) });
  ...
  private boolean shouldTrip()
  {
    ...
    double percentageFreeMemory = 100.0D * ((Runtime.getRuntime().freeMemory() + (Runtime.getRuntime().maxMemory() - Runtime.getRuntime().totalMemory())) / Runtime.getRuntime().maxMemory());
    ...
    Agent.LOG.log(Level.FINEST, "Circuit breaker: percentage free memory {0}%  GC CPU time percentage {1}% (freeMemoryThreshold {2}, gcCPUThreshold {3})", new Object[] { Double.valueOf(percentageFreeMemory), Double.valueOf(gcCpuTimePercentage), Integer.valueOf(freeMemoryThreshold), Integer.valueOf(gcCPUThreshold) });

    if ((gcCpuTimePercentage >= gcCPUThreshold) && (percentageFreeMemory <= freeMemoryThreshold)) {
      Agent.LOG.log(Level.FINE, "Circuit breaker tripped at memory {0}%  GC CPU time {1}%", new Object[] { Double.valueOf(percentageFreeMemory), Double.valueOf(gcCpuTimePercentage) });
  ...
  public void configChanged(String appName, AgentConfig agentConfig)
  {
    ...
    Agent.LOG.log(Level.INFO, "Circuit breaker: updated configuration - enabled {0} GC CPU Threshold {1}% Memory Threshold {2}%.", new Object[] { Boolean.valueOf(this.circuitBreakerConfig.isEnabled()), Integer.valueOf(this.circuitBreakerConfig.getGcCpuThreshold()), Integer.valueOf(this.circuitBreakerConfig.getMemoryThreshold()) });
  }
