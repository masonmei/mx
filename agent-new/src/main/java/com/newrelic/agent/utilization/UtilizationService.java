...
public class UtilizationService extends AbstractService
{
  ...
  private void cleanupAndLogThrowable(Throwable t) {
    MemoryData.cleanup();
    Agent.LOG.log(Level.FINEST, MessageFormat.format("Utilization task exception. Returning cached utilization data. {0}", new Object[] { t }));
  }
  ...
  class UtilizationTask implements Callable<UtilizationData> {
    ...
    private UtilizationData doUpdateUtilizationData() {
      ...
      String containerId = UtilizationService.this.detectDocker ? UtilizationService.this.getDockerContainerId() : null;

      AWS.AwsData awsData = UtilizationService.this.detectAws ? UtilizationService.this.getAwsData() : AWS.AwsData.EMPTY_DATA;
