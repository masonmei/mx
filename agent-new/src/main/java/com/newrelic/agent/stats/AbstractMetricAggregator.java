  ...
  implements MetricAggregator
{
  ...
  public final void recordResponseTimeMetric(String name, long totalTime, long exclusiveTime, TimeUnit timeUnit)
  {
    ...
    try {
      ...
      this.logger.log(Level.FINER, "Recorded response time metric \"{0}\": {1}", new Object[] { name, Long.valueOf(totalTime) });
    } catch (Throwable t) {
      logException(this.logger, t, "Exception recording response time metric \"{0}\": {1}", new Object[] { name, t });
    }
  }

  protected abstract void doRecordResponseTimeMetric(String paramString, long paramLong1, long paramLong2, TimeUnit paramTimeUnit);
  ...
  public final void incrementCounter(String name, int count)
  {
    ...
    try {
      ...
      this.logger.log(Level.FINER, "incremented counter \"{0}\": {1}", new Object[] { name, Integer.valueOf(count) });
    } catch (Throwable t) {
      logException(this.logger, t, "Exception incrementing counter \"{0}\",{1} : {2}", new Object[] { name, Integer.valueOf(count), t });
    }
  }
