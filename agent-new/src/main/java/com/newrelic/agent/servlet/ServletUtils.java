...
public class ServletUtils
{
  ...
  {
    if ((throwable != null) && 
      (SERVLET_EXCEPTION_CLASS_NAME.equals(throwable.getClass().getName()))) {
  ...
  static Map<String, String> getRequestParameterMap(Request request, int maxSizeLimit) {
    ...
    while (nameEnumeration.hasMoreElements()) {
      ...
      if (name.length() > maxSizeLimit) {
        Agent.LOG.log(Level.FINER, "Rejecting request parameter with key \"{0}\" because the key is over the size limit of {1}", new Object[] { name, Integer.valueOf(maxSizeLimit) });
      }
      else
      {
