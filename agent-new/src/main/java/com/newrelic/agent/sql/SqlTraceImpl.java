...
class SqlTraceImpl
  implements SqlTrace, JSONStreamAware
{
  ...
  public void writeJSONString(Writer out)
    throws IOException
  {
    JSONArray.writeJSONString(Arrays.asList(new Serializable[] { this.blameMetricName, this.uri, Integer.valueOf(this.id), this.sql, this.metricName, Integer.valueOf(this.callCount), Long.valueOf(this.total), Long.valueOf(this.min), Long.valueOf(this.max), DataSenderWriter.getJsonifiedCompressedEncodedString(this.parameters, out) }), out);
  }
}