...
public class TomcatJmxValues extends JmxFrameworkValues {
    ...
    private static final JmxMetric SESSION_ALIVE_TIME =
            ServerJmxMetricGenerator.SESSION_AVG_ALIVE_TIME.createMetric("sessionAverageAliveTime");
    ...
    private void createMetrics(String name) {
        this.metrics.add(new BaseJmxValue(name + ":type=Manager,context=*,host=*,*", "JmxBuiltIn/Session/{context}/",
                                                 new JmxMetric[] {ACTIVE_SESSIONS, EXPIRED_SESSIONS, REJECTED_SESSIONS,
                                                                         SESSION_ALIVE_TIME}));

        this.metrics.add(new BaseJmxValue(name + ":type=Manager,path=*,host=*", "JmxBuiltIn/Session/{path}/",
                                                 new JmxMetric[] {ACTIVE_SESSIONS, EXPIRED_SESSIONS, REJECTED_SESSIONS,
                                                                         SESSION_ALIVE_TIME}));

        this.metrics.add(new BaseJmxValue(name + ":type=ThreadPool,name=*", "JmxBuiltIn/ThreadPool/{name}/",
