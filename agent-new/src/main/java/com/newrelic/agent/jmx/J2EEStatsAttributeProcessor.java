...
public class J2EEStatsAttributeProcessor extends AbstractStatsAttributeProcessor {
    private static void pullJMSStats(StatsEngine statsEngine, JMSStats jmsStats, Attribute attribute, String metricName,
    ...
    private static void pullJCAStats(StatsEngine statsEngine, JCAStats jcaStats, Attribute attribute, String metricName,
                                     Map<String, Float> values, boolean isBuiltInMetric) {
    ...
    private static void grabBaseStats(StatsEngine statsEngine, javax.management.j2ee.statistics.Stats jmxStats,
                                      Attribute attribute, String metricName, Map<String, Float> values,
                                      boolean isBuiltInMetric) {
    ...
    static void processStatistic(StatsEngine statsEngine, String metricName, Attribute attribute, Statistic statistic) {
        ...
        Agent.LOG.finer(MessageFormat.format("Processing J2EE statistic: {0} class: {1}",
                                                    new Object[] {statistic.getName(),
        ...
        if ((statistic instanceof CountStatistic)) {
            ...
            statsEngine.getResponseTimeStats(fullMetricName)
                    .recordResponseTime((int) stat.getCount(), stat.getTotalTime(), stat.getMinTime(),
                                               stat.getMaxTime(), unit);
        } else {
            Agent.LOG.log(Level.FINEST, "Not supported: {0}", new Object[] {statistic.getClass().getName()});
        }
        Agent.LOG.finer(MessageFormat.format("Processed J2EE statistic: {0} att: {1}",
                                                    new Object[] {fullMetricName, statistic.getName()}));
    }
    ...
    static boolean addJmxValue(Attribute attribute, Statistic statistic, Map<String, Float> values) {
        if (attribute.getName().contains(statistic.getName())) {
            Agent.LOG.finer(MessageFormat.format("Adding J2EE statistic to List: {0} class: {1}",
            ...
            if ((statistic instanceof BoundaryStatistic)) {
                BoundaryStatistic stat = (BoundaryStatistic) statistic;
                values.put(attribute.getName(),
                                  Float.valueOf((float) ((stat.getLowerBound() + stat.getUpperBound()) / 2L)));
                return true;
            }
    ...
    public boolean process(StatsEngine statsEngine, ObjectInstance instance, Attribute attribute, String metricName,
        ...
        Agent.LOG.finer(MessageFormat.format("Attribute value is not a javax.management.j2ee.statistics.Stats: {0}",
