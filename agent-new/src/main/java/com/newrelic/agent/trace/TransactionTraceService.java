...
public class TransactionTraceService extends AbstractService implements HarvestListener, TransactionListener,
    ...
    private ITransactionSampler getOrCreateTransactionTraceBucket(String appName) {
        ...
        if (bucket == null) {
            ...
            ITransactionSampler oldBucket =
                    (ITransactionSampler) this.transactionTraceBuckets.putIfAbsent(appName, bucket);
            if (oldBucket != null) {
                return oldBucket;
            }
        }
    ...
    private List<TransactionTrace> getAllExpensiveTransactions(String appName) {
        ...
        List<ITransactionSampler> allBuckets =
                new ArrayList(Arrays.asList(new ITransactionSampler[] {this.requestTraceBucket,
                                                                              this.backgroundTraceBucket}));
    ...
    public void xraySessionRemoved(XRaySession session) {
        ...
        if (null != session) {
            XRayTransactionSampler samplerToRemove =
                    (XRayTransactionSampler) this.xraySamplers.remove(session.getxRayId());
