...
public class InstrumentUpdateCommand extends AbstractCommand
{
  ...
  public Map process(IRPMService pRpmService, Map pArguments) throws CommandException
  {
    ...
    if ((pArguments == null) || (pArguments.size() == 0)) {
      Agent.LOG.warning(MessageFormat.format("The instrumentation_update command must have atleast one argument called {0}", new Object[] { "instrumentation" }));
    ...
    if (xml != null)
    {
      ReinstrumentResult result = this.service.processXml(xml);
      if (result != null) {
  ...
  private static void warnIfArgsLeft(Map pArguments) {
    if (pArguments.size() > 0)
      Agent.LOG.warning(MessageFormat.format("The instrumentation_update command did not recognize the following arguments: {0}", new Object[] { pArguments.keySet().toString() }));
  }
  ...
  private static String getXml(Object xml) {
    if (xml != null) {
      ...
      Agent.LOG.info(MessageFormat.format("The property {0} was empty meaning no instrumentation update will occur.", new Object[] { "config" }));
    }
    else
    {
