...
import com.newrelic.deps.com.google.common.collect.ImmutableMap.Builder;
...
public final class Agent extends AbstractService
  implements IAgent
{
  ...
  public static void premain(String agentArgs, Instrumentation inst)
  {
    ...
    if (jvmName.contains("Oracle JRockit")) {
      String msg = MessageFormat.format("New Relic agent {0} does not support the Oracle JRockit JVM. Please use a 2.21.x or earlier version of the agent. JVM  is: {1}.", new Object[] { getVersion(), jvmName });
  ...
  public static void main(String[] args) {
    ...
    if (javaVersion.startsWith("1.5")) {
      String msg = MessageFormat.format("Java version is: {0}.  This version of the New Relic Agent does not support Java 1.5.  Please use a 2.21.x or earlier version.", new Object[] { javaVersion });
  ...
  private static void recordPremainTime(StatsService statsService) {
    ...
    Map environmentInfo = ImmutableMap.builder().put("Duration", Long.valueOf(agentPremainTime)).put("Version", getVersion()).put("JRE Vendor", System.getProperty("java.vendor")).put("JRE Version", System.getProperty("java.version")).put("JVM Vendor", System.getProperty("java.vm.vendor")).put("JVM Version", System.getProperty("java.vm.version")).put("JVM Runtime Version", System.getProperty("java.runtime.version")).put("OS Name", System.getProperty("os.name")).put("OS Version", System.getProperty("os.version")).put("OS Arch", System.getProperty("os.arch")).put("Processors", Integer.valueOf(Runtime.getRuntime().availableProcessors())).put("Free Memory", Long.valueOf(Runtime.getRuntime().freeMemory())).put("Total Memory", Long.valueOf(Runtime.getRuntime().totalMemory())).put("Max Memory", Long.valueOf(Runtime.getRuntime().maxMemory())).build();
