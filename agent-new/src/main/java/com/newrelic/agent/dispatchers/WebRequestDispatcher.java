...
public class WebRequestDispatcher extends DefaultDispatcher
  implements WebResponse
{
  ...
  public WebRequestDispatcher(Request request, Response response, Transaction transaction) { super(transaction);

    boolean isLastStatusCodePolicy = ((Boolean)transaction.getAgentConfig().getValue("last_status_code_policy", Boolean.TRUE)).booleanValue();
  ...
  private void recordDispatcherMetrics(String frontendMetricName, TransactionStats stats) {
    ...
    if ((totalTimeMetric != null) && (totalTimeMetric.length() > 0)) {
      stats.getUnscopedStats().getResponseTimeStats(totalTimeMetric).recordResponseTimeInNanos(getTransaction().getTransactionTimer().getTotalTime());
    }

    stats.getUnscopedStats().getResponseTimeStats("WebTransactionTotalTime").recordResponseTimeInNanos(getTransaction().getTransactionTimer().getTotalTime());
  }
  ...
  private void recordApdexMetrics(String frontendMetricName, TransactionStats stats)
  {
    ...
    if (isApdexFrustrating()) {
      ...
      long responseTimeInMillis = getTransaction().getTransactionTimer().getResponseTimeInMilliseconds() + this.externalTimeTracker.getExternalTime();
  ...
  public boolean isApdexFrustrating() {
    ...
    return (getTransaction().getStatus() >= 400) && (!rpmService.getErrorService().isIgnoredError(getTransaction().getStatus(), getTransaction().getReportError()));
  }
