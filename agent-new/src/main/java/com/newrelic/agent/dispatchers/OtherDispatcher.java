...
public class OtherDispatcher extends DefaultDispatcher
{
  ...
  {
    stats.getUnscopedStats().getResponseTimeStats(transactionName).recordResponseTime(getTransaction().getTransactionTimer().getResponseTime(), 0L, TimeUnit.NANOSECONDS);
    ...
    if ((totalTimeMetric != null) && (totalTimeMetric.length() > 0)) {
      stats.getUnscopedStats().getResponseTimeStats(totalTimeMetric).recordResponseTime(getTransaction().getTransactionTimer().getTotalTime(), 0L, TimeUnit.NANOSECONDS);
    }

    stats.getUnscopedStats().getResponseTimeStats("OtherTransaction/all").recordResponseTime(getTransaction().getTransactionTimer().getResponseTime(), getTransaction().getTransactionTimer().getResponseTime(), TimeUnit.NANOSECONDS);

    stats.getUnscopedStats().getResponseTimeStats("OtherTransactionTotalTime").recordResponseTime(getTransaction().getTransactionTimer().getTotalTime(), getTransaction().getTransactionTimer().getTotalTime(), TimeUnit.NANOSECONDS);
