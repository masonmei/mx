...
public class XrayCpuTimeController extends AbstractController
  implements ProfilingTaskController
{
  ...
  protected int doCalculateSamplePeriodInMillis() { int samplePeriod = getSamplePeriodInMillis();
    long nThreadId = getCurrentThreadId();
    long oThreadId = this.threadId;
    this.threadId = nThreadId;
    long endCpuTimeInNanos;
    try { endCpuTimeInNanos = getThreadCpuTimeInNanos();
    } catch (Throwable t) {
    ...
    if (oThreadId == nThreadId) {
      samplePeriod = calculateSamplePeriod(endTimeInNanos - this.startTimeInNanos, endCpuTimeInNanos - this.startCpuTimeInNanos);
    }
