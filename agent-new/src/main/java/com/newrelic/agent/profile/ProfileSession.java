...
public class ProfileSession
{
  ...
  void start() {
    ...
    if (samplePeriodInMillis == durationInMillis) {
      ...
      getLogger().info(MessageFormat.format("Starting profiling session. Duration: {0} ms, sample period: {1} ms", new Object[] { Long.valueOf(durationInMillis), Long.valueOf(samplePeriodInMillis) }));
    ...
    ScheduledFuture handle = scheduler.scheduleWithFixedDelay(new Runnable() {
      public void run() {
        try {
          ...
          String msg = MessageFormat.format("An error occurred collecting a thread sample: {0}", new Object[] { t.toString() });
  ...
  private void report() {
    try {
      ...
      getLogger().info(MessageFormat.format("Profiler finished with {0} samples", new Object[] { Integer.valueOf(this.profile.getSampleCount()) }));
    } catch (Throwable e) {
