...
public class XrayProfileSession
  implements HarvestListener, XRaySessionListener
{
  ...
  private void startProfiling(ProfilerParameters parameters)
  {
    ...
    String msg = MessageFormat.format("Added xray session profiling for {0}", new Object[] { parameters.getKeyTransaction() });
    Agent.LOG.info(msg);
    if (!this.isSuspended)
      start();
  }
  ...
  private void stopProfiling(ProfilerParameters parameters)
  {
    ...
    String msg = MessageFormat.format("Removed xray session profiling for {0}", new Object[] { parameters.getKeyTransaction() });
    Agent.LOG.info(msg);
  ...
  private void start()
  {
    if (this.profileHandle == null) {
      ...
      Agent.LOG.fine(MessageFormat.format("Started xray profiling task delay = {0}", new Object[] { Long.valueOf(delay) }));
    }
  }
