...
public class UtilizationService extends AbstractService {
    ...
    public UtilizationService() {
        ...
        detectAws = ((Boolean) config.getValue("utilization.detect_aws", Boolean.TRUE)).booleanValue();
        detectDocker = ((Boolean) config.getValue("utilization.detect_docker", Boolean.TRUE)).booleanValue();

        hostName = Hostname.getHostname(Agent.LOG, agentConfig);
        isLinux = isLinuxOs();
    }
    ...
    public UtilizationData updateUtilizationData() {
        ...
        try {
            utilizationData = ((UtilizationData) future.get(1000L, TimeUnit.MILLISECONDS));
            future = null;
    ...
    protected String getDockerContainerId() {
        return DockerData.getDockerContainerId(isLinux);
    }
    ...
    class UtilizationTask implements Callable<UtilizationData> {
        ...
        private UtilizationData doUpdateUtilizationData() {
            ...
            String containerId = detectDocker ? getDockerContainerId() : null;

            AWS.AwsData awsData = detectAws ? getAwsData() : AWS.AwsData.EMPTY_DATA;

            long total_ram_mib = MemoryData.getTotalRamInMib();
            return new UtilizationData(hostName, processorCount, containerId, awsData, total_ram_mib);
        }
    }
}