...
public class NettyDispatcherPointCut extends TracerFactoryPointCut {
    ...
    static final String METHOD_DESC =
            "(Lorg/jboss/netty/channel/ChannelHandlerContext;Ljava/net/SocketAddress;Ljava/lang/Object;)V";
    ...
    public NettyDispatcherPointCut(ClassTransformer classTransformer) {
        ...
        alternateDispatchClassName = ((String) ServiceFactory.getConfigService().getDefaultAgentConfig()
                                                       .getValue("class_transformer.netty_dispatcher_class"));
    }
    ...
    public final Tracer doGetTracer(Transaction tx, ClassMethodSignature sig, Object object, Object[] args) {
        ...
        if ((alternateDispatchClassName != null) && (isRequestPresent) && (!isExpectedType)) {
            isExpectedType = object.getClass().getName().equals(alternateDispatchClassName);
        }
        if ((!isExpectedType) || (!isRequestPresent)) {
            Agent.LOG.log(Level.FINEST, "NettyDispatcher: Skipping message {1} recieved for {0}",
                                 new Object[] {object.getClass(), args[2]});
    ...
    Tracer buildTracer(Transaction tx, ClassMethodSignature sig, Object object, Object[] args) {
        ...
        if (rootTracer != null) {
            Agent.LOG.log(Level.FINER, "NettyDispatcher: rootTracer not null. Already in a transaction? {0}->{1}",
                                 new Object[] {tx, rootTracer});
    ...
    private void setTransactionName(Transaction tx) {
        ...
        if ((Agent.LOG.isLoggable(Level.FINER)) && (policy.canSetTransactionName(tx,
                                                                                        TransactionNamePriority
                                                                                                .SERVLET_NAME))) {
            String msg = MessageFormat.format("Setting transaction name to \"{0}\" using Netty Http Decoder",
                                                     new Object[] {"NettyDispatcher"});
    ...
    private Tracer createTracer(Transaction tx, ClassMethodSignature sig, Object object, Request httpRequest,
        ...
        try {
            return new BasicRequestRootTracer(tx, sig, object, httpRequest, httpResponse,
                                                     new SimpleMetricNameFormat("Java/org.jboss.netty.handler.codec"
                                                                                        + ".http"
