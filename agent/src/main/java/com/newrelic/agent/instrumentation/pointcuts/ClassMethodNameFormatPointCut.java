...
public class ClassMethodNameFormatPointCut extends TracerFactoryPointCut {
    ...
    public ClassMethodNameFormatPointCut(MetricNameFormatFactory metricNameFormatFactory, ClassMatcher classMatcher,
        ...
        skipTransactionTrace = ((Boolean) config.getProperty("skip_transaction_trace", Boolean.FALSE)).booleanValue();
        ignoreTransaction = ((Boolean) config.getProperty("ignore_transaction", Boolean.FALSE)).booleanValue();
    }
    ...
    public ClassMethodNameFormatPointCut(MetricNameFormatFactory pMetricNameFormatFactory, ClassMatcher pClassMatcher,
        ...
        setPriority(19);
        metricNameFormatFactory = pMetricNameFormatFactory;
        dispatcher = pDispatcher;
        skipTransactionTrace = pSkipTransactionTrace;
        ignoreTransaction = pIgnoreTransaction;
    }
    ...
    public Tracer doGetTracer(Transaction transaction, ClassMethodSignature sig, Object object, Object[] args) {
        MetricNameFormat format = metricNameFormatFactory.getMetricNameFormat(sig, object, args);

        if (dispatcher) {
            return new OtherRootTracer(transaction, sig, object, format);
        }
        int flags = 2;
        if (!skipTransactionTrace) {
            flags |= 4;
        }
