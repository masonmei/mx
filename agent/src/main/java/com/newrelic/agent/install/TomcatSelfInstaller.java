...
public class TomcatSelfInstaller extends SelfInstaller {
    ...
    public String getStartScript() {
        ...
        return path + (osIsWindows ? ".bat" : ".sh");
    }

    public String getAlternateStartScript() {
        return "/bin/catalina.50" + (osIsWindows ? ".bat" : ".sh");
    }

    public String getLocator() {
        return osIsWindows ? "^rem Guess CATALINA_HOME if not defined(.*)$" : "^(export )?CATALINA_OPTS=(.*)$";
    }

    public String getAlternateLocator() {
        return osIsWindows ? "^rem Suppress Terminate batch job(.*)$" : "^# OS specific support(.*)$";
    }
    ...
    public String getAgentAlreadySetExpr() {
        return osIsWindows ? "^(SET|set) CATALINA_OPTS=%CATALINA_OPTS% \\-javaagent:(.*)$"
