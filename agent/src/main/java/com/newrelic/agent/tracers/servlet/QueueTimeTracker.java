...
public class QueueTimeTracker {
    ...
    private QueueTimeTracker(Request httpRequest, long txStartTimeInNanos) {
        ...
        queueTime = initQueueTime(requestXQueueStartHeader, txStartTimeInNanos);
    }
    ...
    public void recordMetrics(TransactionStats statsEngine) {
        if (queueTime > 0L) {
            ...
            statsEngine.getUnscopedStats().getResponseTimeStats(name.getName()).recordResponseTimeInNanos(queueTime);
        }
    }
}