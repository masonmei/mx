...
public class ExternalTimeTracker {
    ...
    private ExternalTimeTracker(Request httpRequest, long txStartTimeInMillis) {
        ...
        serverTimeTracker = ServerTimeTracker.create(httpRequest, txStartTimeInNanos, queueTime);
        long serverTime = serverTimeTracker.getServerTime();
        externalTime = TimeUnit.MILLISECONDS.convert(queueTime + serverTime, TimeUnit.NANOSECONDS);
    }
    ...
    public void recordMetrics(TransactionStats statsEngine) {
        ...
        serverTimeTracker.recordMetrics(statsEngine);
    }
}