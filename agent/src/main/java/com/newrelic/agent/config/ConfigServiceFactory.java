...
public class ConfigServiceFactory {
    ...
    public static Map<String, Object> getConfigurationFileSettings(File configFile) throws ConfigurationException {
        String msg = MessageFormat.format("New Relic Agent: Loading configuration file \"{0}\"",
                                                 new Object[] {configFile.getPath()});
        Agent.LOG.info(msg);
        try {
            return AgentConfigHelper.getConfigurationFileSettings(configFile);
        } catch (Exception e) {
            msg = MessageFormat
                          .format("An error occurred reading the configuration file {0}. Check the permissions and "
                                          + "format of the file. - {1}",
                                         new Object[] {configFile.getAbsolutePath(), e.toString()});
