...
public class Jboss56JmxValues extends JmxFrameworkValues {
    ...
    private static final JmxMetric ACTIVE_SESSIONS = JmxMetric.create(new String[] {"activeSessions", "ActiveSessions"},
                                                                             "Active", JmxAction.USE_FIRST_RECORDED_ATT,
                                                                             JmxType.SIMPLE);
    private static final JmxMetric SESSION_ALIVE_TIME = JmxMetric.create(new String[] {"sessionAverageAliveTime",
                                                                                              "SessionAverageAliveTime"},
                                                                                "AverageAliveTime",
                                                                                JmxAction.USE_FIRST_RECORDED_ATT,
                                                                                JmxType.SIMPLE);
    ...
    private static final JmxMetric CURRENT_ACTIVE_COUNT =
            ServerJmxMetricGenerator.ACTIVE_THREAD_POOL_COUNT.createMetric("currentThreadsBusy");
    ...
    static {
        METRICS.add(new BaseJmxValue("jboss.web:type=ThreadPool,name=*", "JmxBuiltIn/ThreadPool/{name}/",
                                            new JmxMetric[] {CURRENT_ACTIVE_COUNT, CURRENT_IDLE_COUNT,
                                                                    CURRENT_MAX_COUNT}));

        METRICS.add(new BaseJmxValue("jboss.web:type=Manager,path=*,host=*", "JmxBuiltIn/Session/{path}/",
                                            new JmxMetric[] {ACTIVE_SESSIONS, EXPIRED_SESSIONS, REJECTED_SESSIONS,
                                                                    SESSION_ALIVE_TIME}));
    }
