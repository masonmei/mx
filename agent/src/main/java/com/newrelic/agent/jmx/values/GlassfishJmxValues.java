...
public class GlassfishJmxValues extends JmxFrameworkValues {
    ...
    static {
        METRICS.add(new BaseJmxValue("amx:type=thread-pool-mon,pp=*,name=*", "JmxBuiltIn/ThreadPool/{name}/",
                                            new JmxMetric[] {ServerJmxMetricGenerator.ACTIVE_THREAD_POOL_COUNT
                                                                     .createMetric("currentthreadsbusy.count"),
                                                                    ServerJmxMetricGenerator.MAX_THREAD_POOL_COUNT
                                                                            .createMetric("maxthreads.count"),
                                                                    CURRENT_IDLE_COUNT}));

        METRICS.add(new BaseJmxValue("amx:type=session-mon,pp=*,name=*", "JmxBuiltIn/Session/{name}/",
                                            new JmxMetric[] {ServerJmxMetricGenerator.SESSION_ACTIVE_COUNT
                                                                     .createMetric("activesessionscurrent.current"),
