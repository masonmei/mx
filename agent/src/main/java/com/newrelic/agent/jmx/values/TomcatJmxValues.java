...
public class TomcatJmxValues extends JmxFrameworkValues {
    ...
    private void createMetrics(String name) {
        metrics.add(new BaseJmxValue(name + ":type=Manager,context=*,host=*,*", "JmxBuiltIn/Session/{context}/",
                                            new JmxMetric[] {ACTIVE_SESSIONS, EXPIRED_SESSIONS, REJECTED_SESSIONS,
                                                                    SESSION_ALIVE_TIME}));

        metrics.add(new BaseJmxValue(name + ":type=Manager,path=*,host=*", "JmxBuiltIn/Session/{path}/",
                                            new JmxMetric[] {ACTIVE_SESSIONS, EXPIRED_SESSIONS, REJECTED_SESSIONS,
                                                                    SESSION_ALIVE_TIME}));

        metrics.add(new BaseJmxValue(name + ":type=ThreadPool,name=*", "JmxBuiltIn/ThreadPool/{name}/",
