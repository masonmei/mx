...
public abstract class AbstractMetricAggregator implements MetricAggregator {
    ...
    public final void recordResponseTimeMetric(String name, long totalTime, long exclusiveTime, TimeUnit timeUnit) {
        if ((name == null) || (name.length() == 0)) {
            logger.log(Level.FINER, "recordResponseTimeMetric was invoked with a null or empty name", new Object[0]);
            return;
        }
        try {
            doRecordResponseTimeMetric(name, totalTime, exclusiveTime, timeUnit);
            logger.log(Level.FINER, "Recorded response time metric \"{0}\": {1}",
                              new Object[] {name, Long.valueOf(totalTime)});
        } catch (Throwable t) {
            logException(logger, t, "Exception recording response time metric \"{0}\": {1}", new Object[] {name, t});
        }
    }
    ...
    public final void recordMetric(String name, float value) {
        if ((name == null) || (name.length() == 0)) {
            logger.log(Level.FINER, "recordMetric was invoked with a null or empty name", new Object[0]);
            return;
        }
        try {
            doRecordMetric(name, value);
            logger.log(Level.FINER, "Recorded metric \"{0}\": {1}", new Object[] {name, Float.valueOf(value)});
        } catch (Throwable t) {
            logException(logger, t, "Exception recording metric \"{0}\": {1}", new Object[] {name, t});
        }
    }
    ...
    public final void incrementCounter(String name, int count) {
        if ((name == null) || (name.length() == 0)) {
            logger.log(Level.FINER, "incrementCounter was invoked with a null metric name", new Object[0]);
            return;
        }
        try {
            doIncrementCounter(name, count);
            logger.log(Level.FINER, "incremented counter \"{0}\": {1}", new Object[] {name, Integer.valueOf(count)});
        } catch (Throwable t) {
            logException(logger, t, "Exception incrementing counter \"{0}\",{1} : {2}",
